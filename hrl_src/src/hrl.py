...
import json
from networks.actor_net import ActorNetwork
from networks.critic_net import CriticNetwork
from networks.replay import Replay
from utilities.toolfunc import ToolFunc
from keras import backend as keras
from interface.inter_sim import InterSim
from rewards.reward_hrl import RewardHRL
import time
...
class ReinAcc(object):
    ...
    def __init__(self):
        ...
        self.actor = ActorNetwork(self.tf_sess, 9, self.action_dim, 10, self.tau, self.LRA)
        self.critic = CriticNetwork(self.tf_sess, 9, self.action_dim, 10, self.tau, self.LRC)
        self.buffer = Replay()
    ...
    def load_weights(self):
        ...
        try:
            self.actor.model.load_weights("../weights/actormodel.h5")
            self.critic.model.load_weights("../weights/criticmodel.h5")
            self.actor.target_model.load_weights("../weights/actormodel.h5")
            self.critic.target_model.load_weights("../weights/criticmodel.h5")
    ...
    def update_weights(self):
        ...
        self.actor.model.save_weights("../weights/actormodel.h5", overwrite=True)
        with open("../weights/actormodel.json", "w") as outfile:
            json.dump(self.actor.model.to_json(), outfile)
        self.critic.model.save_weights("../weights/criticmodel.h5", overwrite=True)
    ...
    def update_batch(self, s, a, r, s1):
        ...
        target_q_values = self.critic.target_model.predict(
            [self.batch_new_state, self.actor.target_model.predict(self.batch_new_state)])
    ...
    def update_loss(self):
        ...
        loss = self.critic.model.train_on_batch([self.batch_state, self.batch_action], self.batch_output)
        actor_predict = self.actor.model.predict(self.batch_state)
        actor_grad = self.critic.gradients(self.batch_state, actor_predict)
        self.actor.train(self.batch_state, actor_grad)
    ...
    def get_action(self, state_t, train_indicator):
        ...
        noise = []
        action_ori = self.actor.model.predict(state_t)
    ...
    def launch_train(self, train_indicator=1):  # 1 means Train, 0 means simply Run
        ...
        self.actor = ActorNetwork(self.tf_sess, state_dim, self.action_size, self.batch_size, self.tau, self.LRA)
        self.critic = CriticNetwork(self.tf_sess, state_dim, self.action_size, self.batch_size, self.tau, self.LRC)
