...
class ReinAcc(object):
    ...
    max_steps = 2000
    action_dim = 6          # Steering/Acceleration/Brake
    action_size = 6
    his_len = 20
    state_dim = 21
    ...
    def get_action(self, state, train_indicator):
        # logging.info('...... Getting action ......')
        noise = []
        action_ori = self.obs_actor.model.predict(state)
        b = np.random.dirichlet(np.ones(3))
        ...
        action_h = (action_ori[0][0:3] + np.array(noise[0:3])) / 2.
        action_l = action_ori[0][3:6] + np.array(noise[3:6])
    ...
    def launch_train(self, train_indicator=1):  # 1 means Train, 0 means simply Run
        ...
        for e in range(self.episode_count):
            ...
            while True:
                ...
                h_action = np.argmax(action_t[0][0:3])
                l_acc = action_t[0][h_action + 3]
                reward_t, collision_l, collision_r, collision_f, not_move, not_stop, jerk = \
                    self.reward.get_reward(state_t[0], h_action, l_acc)
                if jerk > max_j:
                    max_j = jerk
                train_time = time.time() - fre_time
                self.sim.update_vehicle(h_action, l_acc, reward_t)
            ...
            if (e + 1) % 100 == 0:
                ...
                results = {'crash': self.crash, 'unfinished': self.not_finish, 'overspeed': self.overspeed,
                           ...
                           'time': self.run_time}
                with open('../results/hrl_new.txt', 'w+') as json_file:
                    jsoned_data = json.dumps(results)
                    json_file.write(jsoned_data)
