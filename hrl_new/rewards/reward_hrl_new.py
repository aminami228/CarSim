...
class HrlReward(object):
    ...
    def get_reward(self, state, h, a, b=0., st=0.):
        self.state = state
        if h == 0:
    ...
    def reward_notmove(self, accel):
        ...
        f = 0.
        # accel = self.Cft_Accel * a
        if self.state[4] > 0.5 and (self.state[14] > Safe_dis) and (self.state[0] < 0.001) \
                and (accel < 0) and (self.state[1] < 0):
            f = 50. * accel
            f -= 500.
            not_move = 1
            return f, not_move
    ...
    def reward_smooth(self, accel):
        ...
        f1 = - 0.1 * (jerk - 15.) if jerk >= 10. else 0.
        f2 = 0.
        return f1 + f2, jerk
    ...
    def reward_clear(self):
        ...
        for dis1, t1, dis2, t2 in zip(crash_l_dis, crash_l_time, crash_r_dis, crash_r_time):
            ...
            elif dis1 > Tho_dis or (dis1 < - 2.):
                l_dis = 0.
            else:
                l_dis = 0.5 * dis1 - 1
            if 0. <= dis2 <= Tho_dis:
                r_dis = 1. / Tho_dis * dis2 - 1.
            elif dis2 > Tho_dis or (dis2 < - 2.):
                r_dis = 0.
            else:
                r_dis = 0.5 * dis2 - 1
            if 0. <= t1 <= Tho_time:
