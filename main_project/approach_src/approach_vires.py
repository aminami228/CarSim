...
from rewards.reward_vires import Reward
import time
...
class ReinAcc(object):
    ...
    def __init__(self):
        ...
        self.app_actor = ActorNetwork(self.tf_sess, self.state_dim, self.action_dim, self.batch_size, self.tau, self.LRA)
        self.app_critic = CriticNetwork(self.tf_sess, 9, self.action_dim, 10, self.tau, self.LRC)
    ...
    def if_exit(self, step, state, collision, not_move, cannot_stop):
        if step >= self.max_steps:
            ...
            self.if_pass = False
            self.if_done = True
        elif state[6] <= 2.0 and (state[0] <= 4.0):
    ...
    def launch_train(self, train_indicator=1):  # 1 means Train, 0 means simply Run
        ...
        # np.random.seed(1337)
        state_t = self.sim.get_state(0.)
        state_dim = state_t.shape[1]
        self.app_actor = ActorNetwork(self.tf_sess, state_dim, self.action_size, self.batch_size, self.tau, self.LRA)
        self.app_critic = CriticNetwork(self.tf_sess, state_dim, self.action_size, self.batch_size, self.tau, self.LRC)
        self.buffer = ReplayBuffer(self.buffer_size)
        self.load_weights()

        self.sim = InterSim(self.Speed_limit - random() * 5.)
        for e in range(self.episode_count):
            ...
            while True:
                ...
                self.sim.update_vehicle(state_t[0][0])
                state_t1 = self.sim.get_state(action_t[0][0])
            ...
            if (e + 1) % 100 == 0:
                ...
                self.sub_overspeed = 0
                self.sub_not_move = 0
                logging.info('Crash: ' + str(self.crash) + ', Not Stop: ' + str(self.cannot_stop) + ', Not Finished: ' +
                             str(self.not_finish) + ', Overspeed: ' + str(self.overspeed) + ', Not Move: ' +
                             str(self.not_move) + ', Success: ' + str(self.success) + ', Loss: ' + str(loss) +
                             ', Time: ' + str(self.run_time) + ', Test: ' + str(self.if_train))
