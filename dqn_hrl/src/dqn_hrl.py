...
class ReinAcc(object):
    ...
    def update_batch(self, s, a, r, s1):
        ...
        self.ch_critic.target_model.predict(self.batch_new_state)
        q1 = self.ch_critic.target_model.predict(
            [self.batch_new_state, np.ones([self.batch_new_state.shape[0], 1])])
        q2 = self.ch_critic.target_model.predict(
            [self.batch_new_state, -np.ones([self.batch_new_state.shape[0], 1])])
        target_q_values = np.array(np.max(np.array([q1[:, 0], q2[:, 0]]), axis=0), ndmin=2).T
    ...
    def update_loss(self):
        ...
        # actor_predict = self.ch_critic.model.predict(self.batch_state)
        self.ch_critic.train(self.batch_state, self.batch_action)
    ...
    def get_action(self, state, train_indicator, gamma, nn):
        if train_indicator:
            ...
            if nn < zz:
                ...
                q1 = self.ch_critic.target_model.predict([state, [[1.]]])
                q2 = self.ch_critic.target_model.predict([state, [[-1.]]])
                action = [[1.]] if q1 >= q2 else [[-1.]]
        else:
            q1 = self.ch_critic.target_model.predict([state, [[1.]]])
            q2 = self.ch_critic.target_model.predict([state, [[-1.]]])
            action = [[1.]] if q1 >= q2 else [[-1.]]
    ...
    def launch_train(self, train_indicator=1):  # 1 means Train, 0 means simply Run
        ...
        for e in range(self.episode_count):
            ...
            while True:
                ...
                if action_t[0][0] > 0:
                    while True:
                        reward_t, collision_l, collision_r, not_move, jerk = \
                    ...
                    r, collision_l, collision_r, not_move, jerk = \
                        self.reward.get_reward(state_t[0], action_t[0][0])
                    self.sim.update_vehicle(action_t[0][0], reward_t)
                    state_t1 = self.sim.get_state()
                not_stop, dan = 0, 0
            ...
            if (e + 1) % 100 == 0:
                ...
                results = {'crash': self.crash, 'unfinished': self.not_finish, 'overspeed': self.overspeed,
                           'stop': self.not_move, 'not_stop': self.not_stop,
                ...
                with open('../results/new3.txt', 'w+') as json_file:
                    jsoned_data = json.dumps(results)
                    json_file.write(jsoned_data)
                if train_indicator:
