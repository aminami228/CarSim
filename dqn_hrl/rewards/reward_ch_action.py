...
class CHReward(object):
    ...
    def get_reward(self, state, action, st=0.):
        self.state = state
        reward, collision_l = 0., 0
        if action > 0.:
            r, c_l, c_r, c_f = self.reward_clear()
            if c_l <= 0:
                r = 99. if self.state[9] >= - self.state[2] else -1.
            else:
                r = -101.
            return r, c_l, 0, 0, 0.
        else:
            if self.state[-12] <= -3. and (self.state[-2] <= -3.) \
                    and (self.state[13] <= self.Inter_Left - Visibility):
                return -101., 0, 0, 1, 0.
            else:
                return -1., 0, 0, 0, 0.
