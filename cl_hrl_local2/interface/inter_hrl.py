...
class InterSim(object):
    ...
    def __init__(self, gamma, visual=False):
        ...
        if gamma == 0 or (gamma >= 1 and (rr < 1. / (gamma+1.))) and (gamma != 2) and (gamma != 6):
            self.cond = 'empty'
            self.lv_ini = -3.
        else:
    ...
    def get_state(self):
        ...
        visibility_l = 4. * ll / int_lower_dis if (int_lower_dis >= 0.5) else Visibility
        visibility_r = 8. * lr / int_lower_dis if (int_lower_dis >= 0.32) else Visibility
        # 4 - 14
        self.state_road = [sl_dis, int_lower_dis, int_center_y, int_center_x, int_upper_dis, pass_dis, start_pos,
                           ll, lr, visibility_l, visibility_r]
        ...
        for v1 in self.lv_poses:
            if self.Inter_Ori['x'] - visibility_l < v1['x'] < self.Inter_Right:
                lv_cand.append(v1)
        for v2 in self.rv_poses:
            if self.Inter_Ori['x'] < v2['x'] < self.Inter_Right + visibility_r:
                rv_cand.append(v2)
        dis_ = - 3.
        veh_no = Focus_No
        crash_l, crash_r = [], []
