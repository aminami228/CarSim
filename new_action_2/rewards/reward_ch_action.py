...
class CHReward(object):
    ...
    def get_reward(self, state, a, b=0., st=0.):
        ...
        r_smooth, jerk, dan1 = self.reward_smooth(accel)
        # r_cft, dan2 = self.reward_cft(accel)
        r_cft, dan2 = 0., 0
        ...
        r = r_dis + r_time + r_speedlimit + r_clerance + r_smooth + r_finish + r_crash + r_notmove + r_stop + r_cft
        return r, collision_l, collision_r, collision_f, not_move, not_stop, jerk, dan1 + dan2
    ...
    def reward_notmove(self, accel):
        ...
        if self.state[-12] <= -3. and (self.state[-2] <= -3.) and (self.state[0] <= 0.001) and (self.state[1] <= 0.):
            if accel < 0.:
                f = 50 * (accel)
                f -= 500.
                not_move = 1
