...
class CHReward(object):
    ...
    def get_reward(self, state, j, st=0.):
        self.state = state
        # accel = self.Cft_Accel * a
        r_smooth, jerk, dan1 = self.reward_smooth(j)
        jerk = j * self.Tau
        ...
        r_notmove, not_move = self.reward_notmove(state[1] + jerk)
        r = r_dis + r_time + r_speedlimit + r_clerance + r_smooth + r_finish + r_crash + r_notmove + r_stop + r_cft
        return r, collision_l, collision_r, collision_f, not_move, not_stop, j, dan1 + dan2
