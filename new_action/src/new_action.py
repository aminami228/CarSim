...
class ReinAcc(object):
    ...
    def get_action(self, state, train_indicator, gamma, nn):
        if train_indicator:
            ...
            action_ori = self.ch_actor.model.predict(state)
            if gamma == 0:
                noise = [train_indicator * max(self.epsilon, 0.) * self.tools.ou(action_ori[0][0], ha * random(), 0.5, 0.3)]
            # if random() > 0.2:
            #     if ha == 1 or state[0][0] <= 0.:
            #         noise = [train_indicator * max(self.epsilon, 0.) * self.tools.ou(action_ori[0][0], 1., 0.5, -0.3)]
            #     else:
            #         noise = [train_indicator * max(self.epsilon, 0.) * self.tools.ou(action_ori[0][0], -1., 0.5, 0.3)]
            else:
                cc = random() - 0.5
                noise = [train_indicator * max(self.epsilon, 0.) * self.tools.ou(action_ori[0][0], cc, 0.5, 0.2)]
            action_l = action_ori[0] + np.array(noise, ndmin=1)
