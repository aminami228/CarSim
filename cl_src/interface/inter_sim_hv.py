...
class InterSim(object):
    ...
    def __init__(self, visual=False):
        ...
        self.lv_poses = []
        lv_locs = 10. * np.array(sample(xrange(-10, 2), self.LV_NO)) + random()
        for x in lv_locs:
            lv_pos = dict()
        ...
        self.rv_poses = []
        rv_locs = 10. * np.array(sample(xrange(-1, 11), self.RV_NO)) + random()
        for x in rv_locs:
            rv_pos = dict()
    ...
    def draw_scenary(self, av, hvs, r):
        if self.Visual:
            ...
            plt.plot([av['x'], av['x']], [av['y'] - 4., av['y']], 'r.-', markersize=10)
            plt.text(av['x'], av['y'], 'a: ' + str(av['accel']) + ', v: ' + str(av['vy']) + ', reward: ' + str(r) +
                     '\n l_dis: ' + str(self.state_hv[1]) + 'r_dis: ' + str(self.state_hv[4]) +
                     ', center_dis: ' + str(self.Inter_Ori - av['y']))
            for v in hvs:
                if v['dir'] == 'L':
                    plt.plot([v['x'] - 4., v['x']], [v['y'], v['y']], 'k.-', markersize=10)
                if v['dir'] == 'R':
                    plt.plot([v['x'], v['x'] + 4.], [v['y'], v['y']], 'k.-', markersize=10)
    ...
    def get_state(self):
        ...
        lr = self.Lane_Right - (self.av_pos['x'] + self.av_size[1] / 2)
        start_pos = self.Start_Pos
        self.state_road = [int_lower_dis, int_center_dis, int_upper_dis, pass_dis, start_pos]
        ...
        if np.isinf(r_min_dis):
            r_min_dis = -1.
            rv_pos['v'] = -1.
        self.state_hv = [lv_pos['v'], l_min_dis, l_y, rv_pos['v'], r_min_dis, r_y]
        self.state = np.array(self.state_av + self.state_road + self.state_hv, ndmin=2)
        return self.state
